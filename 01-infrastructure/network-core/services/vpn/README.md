...
Este diretório documenta a arquitetura, o provisionamento e as políticas de operação do gateway de **VPN de Acesso Remoto (Client-to-Site)**. O serviço foi desenhado para prover acesso seguro, criptografado e autenticado à infraestrutura corporativa e redes de gerência (OAM) para colaboradores remotos.

O núcleo criptográfico e de negociação de chaves é provido pelo **strongSwan**, um *daemon* IPsec robusto e de código aberto. A topologia utiliza o protocolo **IKEv2** (Internet Key Exchange version 2) para o estabelecimento do túnel, combinado com **EAP-MSCHAPv2** para a autenticação de usuários. Esta escolha arquitetural garante alta segurança e interoperabilidade máxima, permitindo que *endpoints* (Windows, macOS, *mobile*) conectem-se nativamente sem a instalação de *softwares* clientes adicionais.
...
* **P (Performance Management):** Otimização de MSS (*Maximum Segment Size*) e MTU (*Maximum Transmission Unit*) via *iptables/nftables* (TCP MSS Clamping) para evitar fragmentação de pacotes e quedas de conexão (*Blackhole* de PMTU). Uso de extensões de *hardware* (AES-NI) no host físico/virtual para acelerar o processamento criptográfico, evitando gargalos de CPU.
...
1.  **Firewall de Entrada (Ingress):** O *host* que roda o strongSwan deve estar acessível externamente (WAN) estritamente nas portas `UDP 500` (IKE) e `UDP 4500` (NAT-T), além de permitir o tráfego do protocolo IP `50` (ESP).
